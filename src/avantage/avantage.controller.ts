...
export class AvantageController {
  ...
  @Patch(':id')
  @ApiOperation({
    summary: 'Mettre à jour un avantage',
    description: 'Met à jour partiellement un avantage existant. Le nom doit rester unique s\'il est modifié.',
  })
  @ApiParam({
    name: 'id',
    type: Number,
    description: 'ID unique de l\'avantage à mettre à jour',
    example: 1,
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      properties: {
        ...
        statusCode: { type: 'number', example: 201 },
        code: { type: 'string', example: 'success' },
        title: { type: 'string', example: 'Avantage mis à jour' },
        message: { type: 'string', example: 'Avantage "SMS illimités Premium" mis à jour avec succès.' },
        data: {
          type: 'object',
          properties: {
            id: { type: 'number', example: 1 },
            nom: { type: 'string', example: 'SMS illimités Premium' },
            valeur: { type: 'number', example: 10 },
            isGratuit: { type: 'boolean', example: false },
            createdAt: { type: 'string', format: 'date-time' },
            updatedAt: { type: 'string', format: 'date-time' },
          },
        },
  ...
  @ApiConflictResponse({
    description: 'Un avantage avec ce nom existe déjà',
    schema: {
      type: 'object',
      properties: {
        message: { type: 'string', example: 'Un avantage avec le nom "SMS illimités Premium" existe déjà' },
        error: { type: 'string', example: 'Conflict' },
        statusCode: { type: 'number', example: 409 },
      },
    },
    ...
    schema: {
      ...
      properties: {
        message: { 
          type: 'array',
          items: { type: 'string' },
          example: ['valeur must be a positive number']
        },
        error: { type: 'string', example: 'Bad Request' },
        statusCode: { type: 'number', example: 400 },
      },
  ...
  async update(
    ...
    @Body() updateAvantageDto: UpdateAvantageDto,
  ) {
    const avantage = await this.avantageService.update(id, updateAvantageDto);
    return new ResponseApi(
      true,
      200,
      'AVANTAGE_UPDATED',
      'Avantage modifié avec succès',
      'L\'avantage a été modifié avec succès',
      avantage
    );
  }
